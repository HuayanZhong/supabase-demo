...
- 从 `<system-reminder>` 标签中提取**所有压缩摘要**（被压缩掉的早期对话记录），具体位置在：
  - `对话摘要（字数过长已折叠）` 内的 `<summary>` 块
  - `All user messages` 列表 → 提取完整用户消息序列
  - `Files and Code Sections` → 提取涉及的关键文件和代码段
- 将压缩部分的消息数量、工具调用、领域分布计入全量统计
- 提取文件打开记录（了解用户在 IDE 中的关注点）
- 提取 lint 错误记录（如果提供了）
- ⚠️ **注意**：必须从 `<system-reminder>` 标签中完整提取上述内容，确保已压缩轮次不被遗漏。这是全量复盘的基础前提，不要跳过。

### 1.8 Skill 调用记录

- 遍历 AI 的所有 Skill 调用（通过 `Skill()` 工具），记录：
  - 调用的 Skill 名称（如 `supabase`、`nestjs-best-practices` 等）
  - 调用次数
  - 每次调用的上下文（触发原因）
- 统计 Skill 调用总数 `$skill_calls_total` 和按名称分布 `$skill_calls_by_name`
- 评估"应触发但未触发"的场景（对照 `.trae/rules/` 中规则文件绑定的 Skill 匹配条件）
...
```
[{步骤1 收集}] 消息 {N} 轮 | 已压缩 {N} 轮 | 工具调用 {N} 次 | hook 触发 {N} 次 | Subagent 调用 {N} 次 | Skill 调用 {N} 次 | 历史报告 {N} 份
```
