...
统计本会话中规则指针注入后的实际读取情况：

| 规则指针             | 注入次数 | 实际读取 | 命中率 | 说明                       |
| -------------------- | -------- | -------- | ------ | -------------------------- |
| language.md          | —        | —        | —      | 始终生效，无需读取         |
| agent-routing.md     | —        | —        | —      | 每次 UserPromptSubmit 注入 |
| agent-execution.md   | —        | —        | —      | 每次 UserPromptSubmit 注入 |
| agent-roles.md       | —        | —        | —      | SessionStart 注入          |
| agent-quality.md     | —        | —        | —      | Stop 注入                  |
| backend-comments.md  | —        | —        | —      | PreToolUse 按路径注入      |
| frontend-comments.md | —        | —        | —      | PreToolUse 按路径注入      |
| tool/\*.md           | —        | —        | —      | PreToolUse 按工具名注入    |

**计算方法**：

- 注入次数：从 hooks 上下文统计
- 实际读取：检查对话中是否有对应的 Read 工具调用
- 命中率 = 实际读取 / 注入次数 × 100%

**输出**：

- 命中率 < 50% 的规则列表（可能存在过度注入）
- 命中率 = 0% 的规则列表（可能是无效注入）
- 改进建议：是否需要调整注入策略
...
```
[{步骤3 健康}] hook 触发 {N}/{N} | 规则引用 {N}/{N} | 规则命中率 {N}% | 日志完整 {N}/{N} | 盲区 {N} 处
```
