...
- **清理时机**：不依赖外部调度，而是在每次 append 新轮次前自动检查，见下方"持久化前清理检查"

### 持久化前清理检查

每次写入新轮次前，自动执行以下检查，非外部定时任务。

```
[LOOP:append]  START | 检查 JSONL     | path=timer-{id}.jsonl
[LOOP:append]  OK    | 行数未超限     | lines=120;limit=5000;action=直接追加
[LOOP:append]  WARN  | 行数超限       | lines=5012;limit=5000;action=触发归档
[LOOP:append]  WARN  | 文件超龄       | age=32d;limit=30d;action=触发归档
```

**触发归档时的完整流程：**

```
[LOOP:append] WARN   | 触发归档       | trigger=lines≥5000;path=timer-{id}.jsonl
[LOOP:archive] START | 开始归档       | source=timer-{id}.jsonl
[LOOP:archive] MOVE  | 移入 archive   | from=loop-state/;to=loop-state/archive/{id}-{date}.jsonl
[LOOP:archive] CREATE| 新建空 JSONL   | path=timer-{id}.jsonl
[LOOP:archive] DONE  | 归档完成       | old_lines=5012;new_file=空
                     ↓
            继续追加本轮数据到新文件
```

此检查嵌入在执行引擎的 `[LOOP:timer]` 和 `[MEM:write]` 之间，是每次轮次的固定步骤，不依赖外部 cron 或人工检查。
