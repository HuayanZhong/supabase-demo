...
> **Skill 强注入**：审计过程中的每个步骤（步骤 1-9）**必须**先调用 `Skill("TRAE-code-review")` 获取结构化的审计方法论框架，然后按框架执行对应审计逻辑。未调用 TRAE-code-review 的审计步骤视为无效。详见各步骤文件开头。
...
AGENTS.md ──────────→ .trae/agents/       是否收录了全部 Subagent
...
commands/README.md ─→ .trae/agents/       新增：Subagent 审计步骤

.trae/skills/ ──────→ .agents/skills/     junction 目标 vs 目标存在性

agents/README.md ───→ .trae/agents/       Subagent 文件 vs README 清单
agents/README.md ───→ agents/logs/         日志目录存在性
```
...
- 重新检查已修复项
- 确认修复幂等性
- **调用 `Task(subagent_type=general_purpose_task)` 加载 `review-verifier.md` 审查修复结果**（修复涉及规则文件、hooks 配置或 AGENTS.md 时强制触发，纯文案修复可选跳过）
- 输出验证摘要

> 审计修复也是"改动"，因此修复后必须经过 review-verifier 审查，形成闭环。此处触发仅审查修复结果，不审查完整的审计流程。
...
| #   | 步骤         | 详细指令文件                 | Skill 注入                     | 输出摘要格式                                                                                           |
| --- | ------------ | ---------------------------- | ------------------------------ | ------------------------------------------------------------------------------------------------------ |
| 1   | Skills 审计  | `audit/01-skills.md`         | `TRAE-code-review`（审计框架） | `[{步骤1 Skills}] 已安装 {N} 个 \| 新建 junction {N} 个 \| ...`                                        |
| 2   | MCP 审计     | `audit/02-mcp.md`            | `TRAE-code-review`（审计框架） | `[{步骤2 MCP}] 已注册 {N} 个 \| 工具描述正常 {N} 个 \| 异常 {N} 个`                                    |
| 3   | 规则审计     | `audit/03-rules.md`          | `TRAE-code-review`（审计框架） | `[{步骤3 规则}] 总 {N} 个 \| frontmatter缺失 {N} 个 \| 引用异常 {N} 处 \| ... \| agent配置异常 {N} 处` |
| 4   | Hooks 审计   | `audit/04-hooks.md`          | `TRAE-code-review`（审计框架） | `[{步骤4 Hooks}] 事件绑定 {N} 个 \| .ps1已注册 {N} / 孤立 {N} \| ...`                                  |
| 5   | 交叉引用审计 | `audit/05-crossref.md`       | `TRAE-code-review`（审计框架） | `[{步骤5 交叉引用}] AGENTS↔hooks {N} \| ...`                                                           |
| 6   | 配置审计     | `audit/06-config.md`         | `TRAE-code-review`（审计框架） | `[{步骤6 配置}] 新增 {N} 项 \| 冗余 {N} 项`                                                            |
| 7   | 修复验证     | `audit/07-verify.md`         | `TRAE-code-review`（审计框架） | `[{步骤7 验证}] 通过 {N} 项 \| 失败(转人工) {N} 项`                                                    |
| 8   | 语义检查     | `audit/08-semantic-check.md` | `TRAE-code-review`（审计框架） | `[{步骤8 语义检查}] Markdown {N} \| 脚本 {N} \| matcher {N} \| Skills {N} \| 规则 {N}`                 |
| 9   | 输出报告     | `audit/09-report.md`         | `TRAE-code-review`（审计框架） | 按模板输出完整 markdown 报告                                                                           |
