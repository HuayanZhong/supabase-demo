...
└── review-verifier.md         # P3 - 审查验证（双Agent循环）
```
...
| 本目录的 7 个自定义 Subagent                                                                                                                 | `.trae/agents/*.md` | 同样通过 `Task(subagent_type=...)` 调用，按场景匹配 description |

> 路由规则见 `.trae/rules/agent/routing.md` 的 `Agent 选型` 章节和 `.trae/rules/agent/skill-triggers.md`。
