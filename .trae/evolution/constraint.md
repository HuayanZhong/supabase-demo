...
- 变更生效后的 **3 次同类任务** 或 **14 天日历超时** 作为验证期，以先到者为准
- 如果验证期内：
  - 同类失败次数没有减少，或反而增加 → 自动回滚变更
  - 引入了新的失败模式 → 自动回滚变更
  - 效果有显著改善（失败减少 ≥ 50%）→ 标记为"已验证"
  - 14 天超时且同类任务不足 3 次 → 按现有数据判定，无足够数据则自动回滚
- 回滚后系统输出回滚报告，说明回滚原因及建议的替代方案
- 回滚操作同样使用 Two-Phase Commit，确保原子性
...
使用三层递进式检测：关键词对立 → 范围重叠 → LLM 辅助语义判定。

1. **关键词对立检测**（快速筛选） — 检查提案中是否出现以下对立词组组合（与目标文件中已有的规则对比）：
   ...
   | "必须放 / 只允许" | "禁止放 / 不得在" | 文件位置对立            |
   | "自动 / 立即"     | "人工 / 审批后"   | 自动化级别对立          |

2. **范围重叠检测**（结构化比对） — 如果提案和现有规则针对同一文件类型 / 同一目录 / 同一命令，检查规则方向是否一致

3. **LLM 辅助语义判定**（深度检测） — 对关键词和范围检测均未发现矛盾、但 `target_file` 重叠的规则对，执行以下流程：

   ```
   提取提案规则文本 + 同文件中现有规则文本
       ↓
   构建对比 prompt："以下两条规则是否存在语义冲突？规则A：{提案} 规则B：{现有}"
       ↓
   LLM 返回判定：{conflict: true/false, reason: "说明", confidence: 0-1}
       ↓
   ├── conflict=true 且 confidence ≥ 0.8 → 阻止提案，标记"语义矛盾"
   ├── conflict=true 且 confidence < 0.8 → 标记"可疑矛盾"，暂缓自动应用
   └── conflict=false → 通过
   ```
...
4. **检出矛盾后的动作**：
   ...
   - LLM 判定结果记录到 `.trae/memory/changelog/contradiction-log.json`，供后续 evolution 参考学习

5. 安全类约束不与任何规则比较——安全约束永远优先
