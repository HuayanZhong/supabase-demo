...
PreToolUse(execute_sql) → protect-sql.ps1（安全拦截）
  │
PreToolUse(chrome-devtools) → inject-credentials.ps1（注入凭证状态）
  │
PreToolUse(MCP 工具) → inject-tool-rules.ps1（按工具名注入工具规则）
  │
PreToolUse(RunCommand) → validate-commit-msg.ps1（验证 commit message）
  │
  ▼
[工具执行]
  │
...
| PreToolUse(chrome-devtools)         | inject-credentials.ps1     | 注入凭证 | 注入凭证状态（不暴露明文密码）                          | 浏览器操作前 |
