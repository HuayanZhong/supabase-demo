...
- 启用 URI 版本控制后（`app.enableVersioning({ type: VersioningType.URI })`），Swagger 生成器不感知版本路由，需在 `main.ts` 中手动维护路径映射表：
  ...
  // 遍历 document.paths 重写路径
  ```

- **新增 vN Controller 时，必须同步更新 `PATH_VERSION_MAP`**，否则 Swagger 文档路径缺少版本前缀
- 健康检查等不需要版本控制的接口使用 `VERSION_NEUTRAL`：`@Controller({ path: "health", version: VERSION_NEUTRAL })`
