...
| #   | 触发场景（宽匹配）                                                                                                             | Skill                     | 命中标志                                                                               |
| --- | ------------------------------------------------------------------------------------------------------------------------------ | ------------------------- | -------------------------------------------------------------------------------------- |
| 1   | **任何 NestJS 代码操作**（Controller/Service/Module/Guard/Filter/Interceptor/中间件/DTO/Pipe/Entity 的新建、修改、审查、重构） | `nestjs-best-practices`   | 出现 NestJS 装饰器或 `class-validator` import，或用户提及 dto/entity/pipe/guard/filter |
...
| "修改 create-quote.dto.ts 的约束"        | `nestjs-best-practices`                    | DTO 修改 + class-validator       |
| "写个新的 Entity"                        | `nestjs-best-practices` + `mikro-orm-docs` | Entity 既属 NestJS 又属 MikroORM |
