...
以下场景**必须**调用对应 Skill 辅助执行：

| 触发场景                             | Skill                     | 说明                              |
| ------------------------------------ | ------------------------- | --------------------------------- |
| 讲解/编写 NestJS 核心基础原理        | `nestjs-best-practices`   | 涵盖 DI、生命周期、作用域、模块等 |
| 编写 MikroORM Entity/Repository/迁移 | `mikro-orm-docs`          | 确保正确的装饰器和 API 写法       |
| 数据库表设计/索引/查询优化           | `postgresql-table-design` | PostgreSQL 最佳实践               |
| 操作 Supabase 相关功能               | `supabase`                | Auth/DB/Storage/Edge Functions    |
| 编写/修改 Nuxt 页面或组件            | `nuxt`                    | SSR、数据获取、路由等             |
| 使用 Nuxt UI 组件库                  | `nuxt-ui`                 | 组件参数、主题、示例              |

> Skill 通过 `Skill()` 工具在会话中直接调用，不创建子 Agent。
> 如果当前任务同时命中多个 Skill，按命中场景的先后顺序串行调用。
