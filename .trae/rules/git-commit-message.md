...
使用中文编写提交信息。技术术语、包名、API 名保留英文原文。
...
- 以动词开头，使用中文
- 准确反映变更内容，而非仓促概括
- ⚠️ 中文描述无需考虑大小写；英文描述首字母小写

## 脚注

- `BREAKING CHANGE: <描述>` — 不兼容变更，必须标注
- `Refs: #<issue编号>` — 关联的 issue
...
docs: 补充本地开发环境搭建步骤
BREAKING CHANGE: User API 响应格式从扁平结构改为嵌套结构
```

## 约束

- 标题行（类型 + 范围 + 描述）不超过 72 个字符
- 正文如需补充说明，空一行后撰写，每行不超过 72 个字符
- revert commit 保留 git 自动生成的格式，无需手动调整
- squash merge 时合并多个 commit 的描述到一行
